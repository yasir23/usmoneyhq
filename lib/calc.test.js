...
// paycheck: $75k single TX biweekly -> gross 2884.62; net > 2200
...
// debt payoff: $10k @ 18% with $400/mo -> months ~31-33, total interest > 2000
...
// dti: $8k income, $1800 housing, $700 other -> front 22.5%, back 31.25%
...
// pmi: $400k, 10% down, 6.5%, 30yr -> loan 360k, PMI > 0, cancels around month 130-140
...
assert.ok(pmi.monthsUntilCancel > 100 && pmi.monthsUntilCancel < 180, `cancel ${pmi.monthsUntilCancel}`);
// 20% down -> no PMI
assert.strictEqual(pmiCalculator(400000, 20, 6.5, 30).hasPMI, false);

// heloc: $50k @ 7% interest-only -> 291.67/mo; amortized 10yr -> higher
